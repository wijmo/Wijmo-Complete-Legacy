...
$.widget("ui.wijwizard", {
	options: {
		...
		///		Setting the event data's cancel property to false may prevent moving to next panel.
		///	</summary>
		validating: null
	},
	...
	_createButtons: function(){
		...
		if (!this.buttons){
			...
			this.buttons = $('<div/>');
			this.buttons.addClass('ui-wijwizard-buttons');
	...
	_pageLize: function(init){
		...
		if (init){
			...
			this.element.addClass('ui-wijwizard ui-widget ui-helper-clearfix');
			if (this.list){
				this.list.addClass('ui-widget ui-helper-reset ui-wijwizard-steps ui-helper-clearfix');
				this.lis.addClass('ui-widget-header ui-corner-all');
			}
			this.container = $('<div/>');
			this.container.addClass('ui-wijwizard-content ui-widget ui-widget-content ui-corner-all');
			this.container.append(this.panels);
			this.container.appendTo(this.element);
			this.panels.addClass('ui-wijwizard-panel ui-widget-content');
			...
			if (o.activeIndex === undefined) {
				...
				if (typeof o.activeIndex != 'number' && this.panels.filter('.ui-wijwizard-actived').length) {
					o.activeIndex = this.panels.index(this.panels.filter('.ui-wijwizard-actived'));
				}
			...
			this.panels.addClass('ui-wijwizard-hide');
			if (o.activeIndex >= 0 && this.panels.length) { // check for length avoids error when initializing empty pages
				this.panels.eq(o.activeIndex).removeClass('ui-wijwizard-hide').addClass('ui-wijwizard-actived');
	...
	_refreshStep: function(){
		...
		if (this.lis){
			this.lis.removeClass('ui-priority-primary').addClass('ui-priority-secondary');
			if (o.activeIndex >= 0 && o.activeIndex <= this.lis.length - 1){
				if (this.lis){
					this.lis.eq(o.activeIndex).removeClass('ui-priority-secondary').addClass('ui-priority-primary');
				}
				if (this.scrollWrap){
	...
	destroy: function() {
		...
		if (this.list){
			this.list.removeClass('ui-widget ui-helper-reset ui-wijwizard-steps ui-helper-clearfix');
		}
			
		if (this.lis){
			this.lis.removeClass('ui-widget-header ui-corner-all ui-priority-primary ui-priority-secondary');
			this.lis.each(function() {
	...
	add: function(index, title, desc) {
		...
		if (this.panels.length == 1) { // after pagelize
			...
			$panel.removeClass('ui-wijwizard-hide');
	...
	remove: function(index) {
		...
		if ($panel.hasClass('ui-wijwizard-actived') && this.panels.length > 1) {
			this.show(index + (index + 1 < this.panels.length ? 1 : -1));
		}
	...
	_showPanel: function(p){
		...
		$show.addClass('ui-wijwizard-actived');
		if ((o.showOption.blind || o.showOption.fade) && o.showOption.duration > 0){
			...
			$show.hide().removeClass('ui-wijwizard-hide') // avoid flicker that way
	...
	_hidePanel: function(p){
		...
		$hide.removeClass('ui-wijwizard-actived');
		if ((o.hideOption.blind || o.hideOption.fade) && o.hideOption.duration > 0){
			...
			$hide.animate(props, o.hideOption.duration || 'normal', function() {
				$hide.addClass('ui-wijwizard-hide');
	...
	show: function(index) {
		...
		var self = this, o = this.options;
		var data = $.extend({}, this._ui(this.panels[o.activeIndex]));
		data.nextIndex = index;
		data.nextPanel = this.panels[index];
		this._trigger('validating', data);
		if (data.cancel) { return this; }
		
		var $hide = this.panels.filter(':not(.ui-wijwizard-hide)'),
